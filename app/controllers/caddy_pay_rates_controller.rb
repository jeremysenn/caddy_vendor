...
#    @caddy_pay_rates = @course.caddy_pay_rates.sort_by {|cpr| cpr.acronym}
    @caddy_pay_rates = current_user.company.caddy_pay_rates.sort_by {|cpr| cpr.acronym}
  end
...
#    @course = @caddy_pay_rate.course
  ...
  private
    ...
    def caddy_pay_rate_params
      params.require(:caddy_pay_rate).permit(:ClubCompanyID, :RankingAcronym, :Type, :NbrHoles, :Payrate, :RankingID, :course_id)
    end
end
