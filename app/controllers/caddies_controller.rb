...
#      format.html {}
  ...
  def pay
    ...
    note = params[:note]
    unless member.blank?
      transfer = Transfer.new(company_id: current_company.id, from_account_id: member.club_account(current_company.id).id, to_account_id: @caddy.account.id, customer_id: member.id, amount: amount, note: note, caddy_fee_cents: amount, caddy_tip_cents: 0)
      if transfer.save
      ...
      transfer = Transfer.new(company_id: current_company.id, from_account_id: current_company.account.id, to_account_id: @caddy.account.id, amount: amount, note: note, club_credit_transaction_id: transaction_id, caddy_fee_cents: amount, caddy_tip_cents: 0)
      if Transfer.save
        redirect_back fallback_location: @caddy, notice: 'Caddy payment submitted.'
      else
        redirect_back fallback_location: @caddy, alert: 'There was a problem creating the caddy payment.'
